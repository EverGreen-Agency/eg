...
// Tipos para as certificações (consolidado do wordpress.ts)
...
export const wordpressService = {
  ...
  // Buscar posts de autoridade por categoria específica
  async getAutoridadePostsByCategory(categoryId: number, page = 1, perPage = 10): Promise<AutoridadePost[]> {
    try {
      const response = await api.get('/autoridade', {
        params: {
          page,
  ...
  // Funções consolidadas do wordpress.ts
  async getCertificacoes(): Promise<Certificacao[]> {
    try {
      const response = await api.get('/certificado', {
        params: {
          per_page: 100,
  ...
  async getSelosEG(): Promise<SeloEG[]> {
    try {
      const response = await api.get('/selo-eg', {
        params: {
          per_page: 100,
          _embed: true
        }
